...
public abstract class CachingIterativeRobotBase extends CachingRobotBase {
    private enum Mode {
        kNone,
        kDisabled,
        kAutonomous,
     ...
     */
    protected CachingIterativeRobotBase(double period) {
        m_period = period;
    ...
    public void enableLiveWindowInTest(boolean testLW) {
        ...
        if (!m_reportedLw && testLW) {
            HAL.report(FRCNetComm.tResourceType.kResourceType_SmartDashboard, FRCNetComm.tInstances.kSmartDashboard_LiveWindow);
            m_reportedLw = true;
        }
