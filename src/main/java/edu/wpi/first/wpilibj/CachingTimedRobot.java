...
public class CachingTimedRobot extends CachingIterativeRobotBase{
    @SuppressWarnings("MemberName")
    static class Callback implements Comparable<TimedRobot.Callback> {
        ...
        public boolean equals(Object rhs) {
            if (rhs instanceof TimedRobot.Callback) {
                return Double.compare(expirationTime, ((TimedRobot.Callback) rhs).expirationTime) == 0;
            }
            return false;
        }
        ...
        @Override
        public int compareTo(TimedRobot.Callback rhs) {
    ...
    private final PriorityQueue<TimedRobot.Callback> m_callbacks = new PriorityQueue<>();

    /** Constructor for TimedRobot. */
    protected CachingTimedRobot() {
        this(kDefaultPeriod);
    }
     ...
     */
    protected CachingTimedRobot(double period) {
        super(period);
    ...
    public final void addPeriodic(Runnable callback, double periodSeconds) {
        m_callbacks.add(new TimedRobot.Callback(callback, m_startTime, periodSeconds, 0.0));
    }
    ...
    public final void addPeriodic(Runnable callback, double periodSeconds, double offsetSeconds) {
        m_callbacks.add(new TimedRobot.Callback(callback, m_startTime, periodSeconds, offsetSeconds));
    }
}
