...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(NAVIGATION_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, NAVIGATION_N1, NAVIGATION_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_navigation::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), NAVIGATION_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), NAVIGATION_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_navigation::init_navigation();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_navigation::precondition(u, p);

        // Compute initial penalty
        icasadi_navigation::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_navigation::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navigation::grad(u, xi, p, grad);
        Ok(())
    };
